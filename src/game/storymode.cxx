...
constexpr LightConfig storyLight(
    ...
    Fxp(-256),                  // x position of object casting shadow
    Fxp(0)                      // y position of object casting shadow
);

LightUpdater storyUpdater(storyLight);
...
void initNextRound(void) {
    ...
    g_StartStoryFrames = STORY_MODE_TIMER;
    g_Game.selectStoryCharacter = true;
    reset_sprites(); // resets sprite colors
    initGoalColors();
...
void storySelectUpdate(void)
{
    ...
    {
        storyUpdater.update(light, shadow);
    ...
    if (!g_GameOptions.debug_display && Gameplay::g_GameState.roundState != Gameplay::ROUND_STATE_PLAYING) {
        ...
        DrawSpriteText(&font, "V.S.", xpos+64, 0, zpos, offset, space);
...
void tallyScore(void) {
    ...
    if (!g_GameOptions.debug_display) {
        SRL::Debug::Print(12, 12, "Round Score:%09d", player->score.points);
        SRL::Debug::Print(12, 14, "Total Score:%09d", player->score.total);
    }
    if (!allOpponentsBeaten()) {
        draw_heart_element(&heart, player, -96, 24, 16);
        draw_star_element(&star, player->score.continues, 32, 24, 16);
    }
    ...
    if (Gameplay::g_GameState.endDelayTimer < Gameplay::LIFE_COUNT_DELAY_TIMEOUT && !allOpponentsBeaten())
    {
        ...
        unsigned int fiveMillions = player->score.total / 5000000;
        if (fiveMillions > player->score.lastFiveMillion)
        {
            if (player->score.continues < 3)
            {
                player->score.continues++;
            }
            player->score.lastFiveMillion = fiveMillions;
        }
    }
    // additional time;
    if (Gameplay::g_GameState.endDelayTimer < Gameplay::LIFE_COUNT_DELAY_TIMEOUT && !allOpponentsBeaten()) {
