...
extern HSLTracker hsl_incSprites[HSL_MAX+1]; // TODO: use array so multiple objects can be calculated at the same time

extern VectorHSL hslSprites[MAX_PALETTE_ENTRIES];

// extern RgbBuff bufferSprites; // debug - delete later
extern SRL::Types::HighColor colorBuffer[MAX_PALETTE_ENTRIES]; // debug - delete later
...
static inline void initNameEntryColors(void) {
	for (unsigned int i = 0; i < FONT_CHARS; i++) {
		...
		hslSprites[color_id].h = (i * 12); // 30 chars * 12 degrees = 360
